	...
	void receiveData(int initX, int initY, double simTime) throws IOException{//受信メソッド
		...
		if(rcvData != null) {
			...
			if((eachData[11].equals("Normal"))) {
				System.out.println("(エッジサーバ受信データ) "+ eachData[0]+eachData[1]+eachData[2]+eachData[3]+eachData[4]+eachData[5]
						+eachData[6]+eachData[7]+eachData[8]+eachData[9]+eachData[10]+ " 状態：" +eachData[13]);
			...
			if(!(eachData[11].equals("Normal")) && !(eachData[11].equals("Decline"))) {//プロトコルへ
				...
				if(eachData[11].equals("Accept")) {
					System.out.println("(エッジサーバ受信データ) "+ "ドローンID " + dport + " " + eachData[11]);
					udp.lisner.resetData();
