...
public class Udp {
	...
	// sendMsgsメソッド
	// マルチキャストパケットの送信を担当
	public void sendData(int dport, String message, int people, double x, double y, double battery, int port, int state)
	{
		try{
				String str =  "ドローンID " + String.valueOf(dport) + " ： " +
	...
	{
		try{
				String str =  "ドローンID " + String.valueOf(dport) + " ： " + "x： " +
								String.valueOf(x) + " " + "y： " +
								String.valueOf(y) + " " + "バッテリ " +
								String.valueOf(battery)+" "+
								String.valueOf(state) + " ";
